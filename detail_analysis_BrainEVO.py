...
ABLATION_KEYS = [
    'no_curriculum',
    'no_elastic_growth',
    'no_ltp_ltd_soft_label',
    'no_personal_extractor',
]


def build_ablation_config(selected):
    selected = selected or []
    unknown = sorted(set(selected) - set(ABLATION_KEYS))
    if unknown:
        raise ValueError(f'Unknown ablation(s): {unknown}. Available: {ABLATION_KEYS}')
    return {key: key in selected for key in ABLATION_KEYS}


def get_ablation_name(config):
    active = [key for key in ABLATION_KEYS if config.get(key, False)]
    if not active:
        return 'full_model'
    return 'abl_' + '+'.join(active)


def random_sort_indices(length, seed=None):
    rng = np.random.RandomState(seed) if seed is not None else np.random
    return rng.permutation(length).tolist()


parser = argparse.ArgumentParser()
parser.add_argument('--dataset', type=str, default=None)
parser.add_argument(
    '--ablation', nargs='*', default=[], choices=ABLATION_KEYS,
    help='选择要开启的消融项，可传多个；不传则运行 full model。'
)
...
print('ABLATION_NAME:', ABLATION_NAME)
print('ABLATION_CONFIG:', ABLATION_CONFIG)
...
new_j = task_per_dir - 1
few_shot_scale = 0.3
for i in range(len(task_dirs)):
    dataloaders, scaler, num_nodes = get_dataloaders_scaler_and_split_task_few_shot(
        task_dirs[i], batch_size, task_per_dir, few_shot_scale, new_j
    )
...
if ABLATION_CONFIG['no_curriculum']:
    sorted_clusters_gradients_list = random_sort_indices(len(cluster_members), seed=args.seed)
    print('[Ablation] no_curriculum: random cluster order')
else:
...
for circle in range(second_stage_circle):
    for cluster_index in sorted_clusters_gradients_list:
        ...
        if ABLATION_CONFIG['no_curriculum']:
            temp_sorted_gradients_list = random_sort_indices(len(cluster), seed=args.seed + circle * 100 + cluster_index)
            sorted_gradients_list = [cluster[index] for index in temp_sorted_gradients_list]
            print('[Ablation] random domain order:', sorted_gradients_list)
        else:
        ...
        for i_j_couple in sorted_gradients_list:
                ...
                if ABLATION_CONFIG['no_elastic_growth']:
                    p_common = 0.5
                    weight_decay_common = 0.5
                    print('[Ablation] no_elastic_growth: keep dropout and weight_decay fixed')
                else:
                ...
                for epoch in tqdm(range(second_stage_num_epochs)):
                    ...
                    if LTD and LTD_bool and saved:
                                ...
                                LTD_bool = False
                    if (not ABLATION_CONFIG['no_ltp_ltd_soft_label']) and epoch % 2 == 0:
                        ...
                        if activate_frequency > 0.2:
                            ...
                            if LTP_bool and saved:
                                ...
                                LTP = True
                                LTP_bool = False
                            if not ABLATION_CONFIG['no_elastic_growth']:
                                weight_decay_common *= 1.2
                                common_model_optimizer = torch.optim.Adam(common_model.parameters(), lr=common_learning_rate, weight_decay=weight_decay_common)
...
#
print('============================Personal Extractor===================================')
third_stage_circle = 5
third_stage_num_epochs = 20
personal_extractor = None
if ABLATION_CONFIG['no_personal_extractor']:
    print('[Ablation] no_personal_extractor: skip contrastive extractor and target-domain relatedness judgment')
else:
    p_personal = 0.1
...
best_mape = 100
best_rmse = 100
if ABLATION_CONFIG['no_personal_extractor'] or min_distance<kappa:
    coupled_model =common_model
    coupled_model.to(device)
    if ABLATION_CONFIG['no_elastic_growth']:
        coupled_model.dropout = p_common
        coupled_weight_decay = weight_decay_common
    else:
    ...
    new_model_dropout = p_common if ABLATION_CONFIG['no_elastic_growth'] else 0.1*p0
    new_model_weight_decay = weight_decay_common if ABLATION_CONFIG['no_elastic_growth'] else 0.1*lambda0
    coupled_model = gwnet(device=device,num_nodes=num_nodes,gcn_bool=True,addaptadj=True,in_dim=in_dim,dropout=new_model_dropout)
    coupled_optimizer = optim.Adam(coupled_model.parameters(), lr=coupled_learning_rate, weight_decay=new_model_weight_decay)
    for epoch in range(fourth_stage_num_epochs):
        coupled_train_loss.append(
            train(new_scaler, new_train_loader, epoch, coupled_optimizer, criterion, coupled_model, device, in_dim))
        mae, rmse, mape = validate(new_scaler, new_val_loader, criterion, coupled_model, device, in_dim)
